...
#define __capacity(self) ((self)->cap)

#define Vec_new() {NULL, 0, 0}

#define Vec_with_capacity(T, num) {malloc(sizeof(T) * (num)), num, 0}
...
#define __grow(self)                                                         \
    ({                                                                       \
        ...
        Vec_as_ptr(self) = realloc(                                          \
            Vec_as_ptr(self), sizeof(*Vec_as_ptr(self)) * __capacity(self)); \
    })
...
#define Vec_sort(self, comp)                                              \
    ({                                                                    \
        qsort(Vec_as_ptr(self), Vec_len(self), sizeof(*Vec_as_ptr(self)), \
              comp);                                                      \
    })
...
#define Vec_first(self) (&Vec_as_ptr(self)[0])

#define Vec_last(self) (&Vec_as_ptr(self)[Vec_len(self) - 1])

#define __vec_swap(a, b)         \
    ({                           \
        int count = sizeof(a);   \
        char *_a = (char *)&(a); \
        char *_b = (char *)&(b); \
...
#define Vec_reverse(self)                                              \
    ({                                                                 \
        ...
        while (i < half_len) {                                         \
            __vec_swap(Vec_as_ptr(self)[i],                            \
                       (&Vec_as_ptr(self)[half])[half_len - 1 - i]);   \
            i++;                                                       \
        }                                                              \
    })
